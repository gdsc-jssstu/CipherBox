    ...
    for (let i = 0; i < len; i += 4) {
        ...
        for (let j = i; j <= i + 3 && j < len; j++) {
            ...
            if (encoded_string[j] >= 'A' && encoded_string[j] <= 'Z') {
                base64_character_index = encoded_string[j].charCodeAt(0) - 'A'.charCodeAt(0);
            } else if (encoded_string[j] >= 'a' && encoded_string[j] <= 'z') {
                base64_character_index = encoded_string[j].charCodeAt(0) - 'a'.charCodeAt(0) + 26;
            } else if (encoded_string[j] >= '0' && encoded_string[j] <= '9') {
                base64_character_index = encoded_string[j].charCodeAt(0) - '0'.charCodeAt(0) + 52;
